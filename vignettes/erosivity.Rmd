...
1. `Wischmeier and Smith` $e_r = 0.119 + 0.0873log_{10}(i)$ with the upper limit of 0.283  MJ/ha/mm if ${i} > 76$ mm/h. [@wischmeier1958rainfall].
2. `Brown and Foster` $e_r = 0.29(1 - 0.72  e^{-0.05i})$ [@brown1987storm].
3. `McGregor et al.` $e_r = 0.273 + 0.2168e^{-0.048i} - 0.4126e^{-0.072i}$ [@McGregor1995].
...
This is an example about using the internal data set to compute rainfall erosivity values.
...
a) Split the time-series to independent storms using the value of 6 hours as the  monthly critical time duration of no precipitation. 
b) Compute the rainfall erosivity values per storm.
c) Filter the above values using only which of them are considered erosive.

```{r "compute erosivity"}
ei_values <- prec5min %>%
  hyet_split(time_step = 5, ts_unit = "mins", crit_dur = rep(6, 12)) %>%
...
After the calculation of erosivity values, the annual value and  of the R coefficient can be computed with:
