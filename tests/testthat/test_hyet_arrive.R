context("hyet_arrival related tests")

test_that("hyet_arrival returns errors", {
...
test_that("hyet_arrival works, units in hours", {
  ...
  arrivals <- hyet_arrive(storms)

  # expect 3 storms
  expect_equal(nrow(arrivals), tail(storms$storm, 1))
})

test_that("hyet_arrival works, units in mins", {
  ...
  # expect the same number of storms
  expect_equal(nrow(arrivals), tail(storms$storm, 1))
})
